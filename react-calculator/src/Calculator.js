import React, { Component } from "react";
//import React from 'react'

export default class Cal extends Component {
  select = (e) => {
    this.props.select(e.target.name);
  };
  render() {
    // These buttons would be better as a component.
    // Even better would be to make an array of your `name` values and map over them!
    // Working on that would also greatly improve your React skills. Try it!
    return (
      <div className="button">
        <button className="pink" name="AC" onClick={this.select}>
          {" "}
          AC{" "}
        </button>
        <button className="pink" name="+/-" onClick={this.select}>
          {" "}
          +/-
        </button>
        <button className="pink" name="%" onClick={this.select}>
          {" "}
          %{" "}
        </button>
        <button className="pink" name="/" onClick={this.select}>
          {" "}
          /{" "}
        </button>

        <br />

        <button name="7" onClick={this.select}>
          {" "}
          7{" "}
        </button>
        <button name="8" onClick={this.select}>
          {" "}
          8{" "}
        </button>
        <button name="9" onClick={this.select}>
          {" "}
          9{" "}
        </button>
        <button className="pink" name="*" onClick={this.select}>
          {" "}
          *{" "}
        </button>
        <br />
        <button name="4" onClick={this.select}>
          {" "}
          4{" "}
        </button>
        <button name="5" onClick={this.select}>
          {" "}
          5{" "}
        </button>
        <button name="6" onClick={this.select}>
          {" "}
          6{" "}
        </button>
        <button className="pink" name="-" onClick={this.select}>
          {" "}
          -{" "}
        </button>
        <br />
        <button name="1" onClick={this.select}>
          {" "}
          1{" "}
        </button>
        <button name="2" onClick={this.select}>
          {" "}
          2{" "}
        </button>
        <button name="3" onClick={this.select}>
          {" "}
          3{" "}
        </button>
        <button className="pink" name="+" onClick={this.select}>
          {" "}
          +{" "}
        </button>
        <br />
        <button name="0" onClick={this.select}>
          {" "}
          0{" "}
        </button>
        <button class="pink" name="." onClick={this.select}>
          {" "}
          .{" "}
        </button>
        <button class="pink" name="Delete" onClick={this.select}>
          {" "}
          Delete{" "}
        </button>
        <button className="pink" name="=" onClick={this.select}>
          {" "}
          ={" "}
        </button>
      </div>
    );
  }
}
